...
name: eval-design
description: >-
  Design evaluation for an AI/LLM system from Product expectations and the
  codebase: a blocked Product intake when decisions are missing, a candidate
  atomic binary rubric for expert review, or per-criterion evaluator routing
  after the rubric is approved. Includes factual architecture notes, a golden
...
Invoke from the project root. Read [references/product-expectations.md](references/product-expectations.md) first and follow it. Output completeness depends on workflow state. A useful intake or review packet is a successful finish.
...
```
- [ ] Step 0: Product intake — read references/product-expectations.md
...
## Step 0: Product intake

Search the repo and the user message for a versioned expectations packet (Markdown plus `product-expectations.json`). If none exists, or required decisions are unresolved, produce the intake in Step 0's output location and set:
...
Include the versioned mode/output/source inventory, applicability map, story packet, and decision log from the contract. Prefill observed system facts from source-pinned code. Mark unknown owner, intent, and consequence as `unknown`.

Do not generate an evaluator, candidate scoring rubric, judge prompt, threshold, or release gate from absent expectations.
...
Classify from code and record one line:
...
- **Expectations present, rubric unapproved** — write `rubric-review.md` and `rubric-register.json`. Each criterion has an ID and version, source expectation, applicable unit, preconditions, one independently failable judgment, pass rule, fail rule, unscorable rule, required evidence, and a proposed consequence whose status is `proposed`. The review record lists the Product/domain approver, named expert, real examples, boundary cases, development dataset ID/version, disagreements, unresolved questions, and `approval_state: pending` until a source supplies approval. Then stop. Do not write judge prompts or implement evaluators.
- **Rubric version approved** — read or write `evaluator-register.json`. Route every approved criterion independently: mechanically computable → deterministic; interpretation, evolving boundaries, or named accountability → named human; stable repeatable semantic judgment → optional LLM. Carry criterion ID/version, evaluator type/ID/version, measurement rationale, acceptance state, scoped evidence, provenance, and decision consequence. Missing approval evidence keeps `acceptance_state` unaccepted and `decision_eligible` false. A bare `approved: true` is not enough.
...
- [ ] `NEEDS_PRODUCT_DECISION` has inventory, applicability map, story packet, decision log, and observed facts. No scoring rubric, judge prompt, threshold, or release gate appears anywhere in the output.
