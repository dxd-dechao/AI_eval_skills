...
The spec chooses the mode and `k`. The harness does not. Each item runs `k` independent trials through the real entrypoint, with fresh state when the app keeps product state. `pass^k` fails if any valid trial fails and passes only when all `k` pass. `pass@k` passes if any valid trial passes and fails only when all `k` fail. Fewer than `k` valid trials is Unscorable (`incomplete trials`). Infrastructure and run-health failures are recorded separately and are not product failures. Per-trial success and `k` are reported. Runs are not compared across different modes or different `k`.
