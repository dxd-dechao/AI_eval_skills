...
The generated runner defaults to diagnostic execution (`decision_eligible=false`, no release verdict). Explicit gate mode preflights, before any scoring, in this order: Eval Spec `READY`; trial contract is `k=1`, `pass^k`, or `pass@k` with a positive integer `k` (`k=1` requires `k` of 1); `runs` equals that `k`; stateful apps have a verified reset or new-instance path; each required criterion's unit equals the spec judgment unit (`output`, `session`, or `episode`). Then approval, accepted evaluator versions, Product consequence, and coverage. A miss exits nonzero with a blocked reason before quality scoring. Zero checks are a blocked configuration, not a pass.

The spec chooses the mode and `k`. The harness does not. Each item runs `k` independent trials through the real entrypoint, with fresh state when the app keeps product state. Decide the item in this order. `k=1` is that trial, or Unscorable (`incomplete trials`) when the trial is run-health. `pass^k` is Fail when any valid trial fails; otherwise Unscorable (`incomplete trials`) when fewer than `k` trials are valid; otherwise Unscorable when any valid trial is unscorable, pending, or error; otherwise Pass. `pass@k` is Pass when any valid trial passes; otherwise Unscorable (`incomplete trials`) when fewer than `k` trials are valid; otherwise Unscorable when any valid trial is unscorable, pending, or error; otherwise Fail. Infrastructure and run-health failures are recorded separately, counted as run health, and are not product failures. Per-trial success and `k` are reported. Runs are not compared across different modes or different `k`.

`output` judges one result. `session` replays ordered turns in one fresh state; turn-level checks are diagnostic and do not enter the session gate. `episode` judges `{trajectory, final_state}` from what the app exposes. A trajectory criterion with no observable trajectory is Unscorable (`trajectory not observable`). The denominator counts judgment units (items), not trials.

Shadow scores stay off the gate. A required unaccepted criterion blocks the decision. One item remains wiring evidence, not a release baseline.
